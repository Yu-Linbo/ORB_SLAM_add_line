...
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp)
{
    ...
    /// 提前纠正畸变图片,特征点检测后不需要再矫正(RGBD 不适用)
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        {
            if(bOK)
                bOK = TrackLocalMapWithLines();  // 修改TrackLocalMap() 为 TrackLocalMapWithLines()，加入线特征进行局部地图特征跟踪
        }
        else
        {
...
//                            mCurrentFrame.mvKeylinesUn[i].endPointX,mCurrentFrame.mvKeylinesUn[i].endPointY,mCurrentFrame.mvDepth_line_end[i];
        ...
        cout << "Initial:  New map created with " << mpMap->MapPointsInMap() << " points；"  << mpMap->MapLinesInMap() << " lines." << endl;
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    int lnmatches = lmatcher.SearchByProjection(mpReferenceKF,mCurrentFrame,vpMapLineMatches);
    ...
    double lmatch_ratio = lnmatchesMap*1.0/mCurrentFrame.mvKeylinesUn.size();
    /// 暂定lmatch_ratio >= 0.5
    return nmatchesMap>=10 || lmatch_ratio >= 0.5;
}
...
/**
 * @brief 在局部地图中查找在当前帧视野范围内的线，将视野范围内的线和当前帧的特征线进行投影匹配
 */
